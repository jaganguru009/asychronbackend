...
router.get('/',(req,res,next )=>{
    ...
    salaryDetailService.getSalDetailByUserName(req.query.userName,(err,result)=>{
        if(err){
...
router.post('/',(req,res,next)=>{
    salaryDetailService.getSalDetailByUserName(req.body.userName,(err,result)=>{
        if(err){
            res.status(500).send(err);
        }else{salaryDetailService.postSalDetail(req.body,(err,result)=>{
            if(err){
                res.json(err);
            }else{
                res.json(result);
