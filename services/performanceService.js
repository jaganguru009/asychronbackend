    ...
    performanceModel.findById(id, (err, result) => {
        ...
        if (err) {
            ...
            if (result != null) {
                ...
                result.codingStandards = performance.codingStandards || result.codingStandards

                result.managerReview =performance.managerReview || result.managerReview
                result.hrReview = performance.hrReview || result.hrReview
                result.teamReview = performance.teamReview || result.teamReview
                result.overallPerformance = performance.overallPerformance || result.overallPerformance

                result.technicalGoals = performance.technicalGoals || result.technicalGoals
                result.managerialGoals = performance.managerialGoals || result.managerialGoals
                result.hrReview = performance.hrReview || result.hrReview
                result.teamGoals = performance.teamGoals || result.teamGoals
                result.feedbackGoals = performance.feedbackGoals || result.feedbackGoals
                ...
                result.save((err, result) => {
                    if (err) {
                        ...
                        callback(null,result);
                        console.log("succesfully updated performance data in your database")
                        return;
                    }
                })
            }else{
